...
function spawnPlayer(){
	...
	mesh = new THREE.Mesh(
		...
		new THREE.MeshNormalMaterial({color:0xFCECA4, wireframe:USE_WIREFRAME})
	);
...
function shiftBlocks(){
	...
	var block = new THREE.Mesh(
			...
			new THREE.MeshNormalMaterial({color:0x00CC2C})
		);
		//left block
		if(rand == 0){
...
function spawnBlock(index){
	...
	if(stack.length != 0){
		block = new THREE.Mesh(
			new THREE.BoxGeometry(10,10,10),
			new THREE.MeshNormalMaterial({color:getRandomColor(), wireframe:USE_WIREFRAME})
		);
		//left block
		if(rand == 0){
		...
		block = new THREE.Mesh(
			new THREE.BoxGeometry(10,10,10),
			new THREE.MeshNormalMaterial({color:0x5E3C58, wireframe:USE_WIREFRAME})
		);
...
function pauseMenu(on){	
	if(on == 1){
		menu = new THREE.Mesh(
			...
			new THREE.MeshNormalMaterial({color:0x000000, wireframe:USE_WIREFRAME, transparent: true, opacity: 0.5})
		);
